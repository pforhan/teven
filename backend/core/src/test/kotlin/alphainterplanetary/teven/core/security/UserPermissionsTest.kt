...
class UserPermissionsTest {
  ...
  fun `isSuperAdmin should return true for superadmin role`() {
    val userPermissions =
      UserPermissions(roles = listOf(Constants.ROLE_SUPERADMIN), permissions = emptyList())
  ...
  fun `isSuperAdmin should return false for other roles`() {
    val userPermissions =
      UserPermissions(roles = listOf("Admin", "User"), permissions = emptyList())
  ...
  fun `hasAllPermissions should return true for superadmin`() {
    ...
    Assertions.assertTrue(
      userPermissions.hasAllPermissions(
        Permission.MANAGE_USERS_GLOBAL,
        Permission.MANAGE_ROLES_GLOBAL
      )
    )
  }
  ...
  fun `hasAllPermissions should return true when all permissions exist`() {
    ...
    Assertions.assertTrue(
      userPermissions.hasAllPermissions(
        Permission.MANAGE_USERS_ORGANIZATION,
        Permission.VIEW_USERS_ORGANIZATION
      )
    )
  }
  ...
  fun `hasAnyPermissions should return true for superadmin`() {
    val userPermissions =
      UserPermissions(roles = listOf(Constants.ROLE_SUPERADMIN), permissions = emptyList())
    Assertions.assertTrue(
      userPermissions.hasAnyPermissions(
        Permission.MANAGE_USERS_GLOBAL,
        Permission.MANAGE_ROLES_GLOBAL
      )
    )
